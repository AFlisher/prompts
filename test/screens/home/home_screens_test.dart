...
void main() {
  ...
  group('PaywallScreen (Buy Credits)', () {
    ...
    // Sprint 2 / B-3: the two simulated-purchase tests that stood here are
    // deleted, not adapted.
    //
    // They asserted that tapping buy showed an imitation of Apple's / Google's
    // own purchase sheet and then displayed "Purchase Successful!" - behaviour
    // that granted credits in device memory, never contacted the server, and
    // violated App Store 3.1.1 and Google Play's payments policy. There is no
    // adapted version of a test for a flow that had to be removed.
    //
    // The real purchase flow is covered in test/screens/paywall_purchase_test.dart,
    // which asserts the properties that replaced them: credits come only from a
    // server response, an unverified purchase never reports success, and a pack
    // the store cannot sell offers no purchase button.
  });
